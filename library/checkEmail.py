...
class emailObject():
    ...
    def __init__(self,imap,imap_port,smtp, smtp_port):
        mydate = datetime.datetime.now()-datetime.timedelta(1)
        self.today = mydate.strftime("%d-%b-%Y")
        self.imap = imaplib.IMAP4_SSL(imap,imap_port)
        self.imap_port = imap_port
        # self.smtp = smtplib.SMTP(smtp,smtp_port)
    ...
    def sendEmailMIME(self, recipient, subject, message):
        ...
        try:
            self.smtp = smtplib.SMTP(self.smtp, self.smtp_port)
            self.smtp.ehlo()
            self.smtp.starttls()
