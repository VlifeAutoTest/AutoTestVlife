...
def writeInfoToXml(input_fn, output_fn, suite_name, only_import_case=False):
    ...
    with open(input_fn) as rfile:
        ...
        for line in reader:
            ...
            (c_product, c_vendor, exe_type, importance, c_module, c_component, case_name, case_summary, preconditions, man_steps, exp_result) = \
            ...
            unicode_str = unicode(case_summary.decode('gbk').encode('utf-8'))
            ...
            etype_text = doc.createCDATASection(str(exe_type))
            ...
            # step
            steps = format_test_steps(doc, man_steps, exe_type, exp_result)
            testcase.appendChild(steps)
            #custom_fields
            cfields = doc.createElement('custom_fields')

            # custom_field name
            dict = {'Product':c_product,'Vendor':c_vendor, 'Component':c_component, 'Module':c_module}
