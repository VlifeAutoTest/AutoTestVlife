...
def format_custom_fields(custom_obj, dom):
    ...
    for cf in custom_fields:
        ...
        if n_value == u"测试分类":
            ...
            c_child_func = v_value

    return c_category, c_component, c_func, c_child_func
...
def parseXmlToCsv(infile,outfile):
    ...
    with open(outfile,"wb+") as wfile:
        ...
        for tc in testcases:
            ...
            c_category, c_component, c_func, c_child_fun = format_custom_fields(custom_fields, dom)

            line = [c_category, c_component, c_func, c_child_fun, case_name,\
                    preconditions_text, importance_text, steps_text, expe_result_text, estimate_text]
